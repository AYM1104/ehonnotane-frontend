...
// // MARK: - X（Twitter）認証プロバイダー
// class TwitterAuthProvider: ObservableObject, AuthProvider {
...
//         completion(AuthResult(success: false, provider: .twitter, error: NSError(domain: "Auth0", code: -1, userInfo: [NSLocalizedDescriptionKey: "Auth0モジュールが利用できません"])))
//         #endif
//     }
...
//                     case .success:
//                         self?.clearAuthState()
//                         print("✅ X（Twitter）ログアウト完了")
//                         completion(true)
                        
//                     case .failure(let error):
//                         self?.errorMessage = "X（Twitter）ログアウトに失敗しました: \(error.localizedDescription)"
//                         print("❌ X（Twitter）ログアウトエラー: \(error)")
//                         completion(false)
//                     }
//                 }
//             }
...
//             var userFriendlyMessage = "X（Twitter）ログインに失敗しました"
...
//                 5. Twitter Developerで作成したAPI KeyとAPI Secretを設定
                
//                 詳細は開発者にお問い合わせください。
//                 """
//                 print("❌ X（Twitter）ログインエラー: Auth0でTwitter接続が有効になっていません")
//                 print("   解決方法: Auth0ダッシュボードでTwitter接続を有効化してください")
//             } else {
//                 userFriendlyMessage = "X（Twitter）ログインに失敗しました: \(errorDescription)"
//             }
            
//             errorMessage = userFriendlyMessage
//             print("❌ X（Twitter）ログインエラー詳細: \(error)")
