...
// MARK: - Google認証プロバイダー
class GoogleAuthProvider: ObservableObject, AuthProvider {
            ...
            .clearSession(federated: false) { [weak self] result in
                DispatchQueue.main.async {
                    ...
                    case .failure(let error):
                        self?.authManager?.errorMessage = "Googleログアウトに失敗しました: \(error.localizedDescription)"
    ...
    private func handleAuthResult(_ result: Auth0.WebAuthResult<Auth0.Credentials>, completion: @escaping (AuthResult) -> Void) {
        ...
        case .failure(let error):
            authManager?.isLoggedIn = false
            authManager?.errorMessage = "Googleログインに失敗しました: \(error)"
